...
describe('Boundaries tasks tests', () => {
  ...
  it('Multiple parallel task runs with boundaries', async () => {
    ...
    expect(sortedRecords[0].boundaries.fetchExternalData.calls).toHaveLength(1)
    expect(sortedRecords[0].boundaries.fetchExternalData.calls[0].input).toEqual([2])
    expect(sortedRecords[0].boundaries.fetchExternalData.calls[0].output).toBe(4)

    // Check record for second task (value: 3)
    expect(sortedRecords[1].input).toEqual({ value: 3 })
    expect(sortedRecords[1].output).toBe(18)
    expect(sortedRecords[1].boundaries.fetchExternalData.calls).toHaveLength(1)
    expect(sortedRecords[1].boundaries.fetchExternalData.calls[0].input).toEqual([3])
    expect(sortedRecords[1].boundaries.fetchExternalData.calls[0].output).toBe(6)

    // Check record for third task (value: 4)
    expect(sortedRecords[2].input).toEqual({ value: 4 })
    expect(sortedRecords[2].output).toBe(32)
    expect(sortedRecords[2].boundaries.fetchExternalData.calls).toHaveLength(1)
    expect(sortedRecords[2].boundaries.fetchExternalData.calls[0].input).toEqual([4])
    expect(sortedRecords[2].boundaries.fetchExternalData.calls[0].output).toBe(8)
  })
})
